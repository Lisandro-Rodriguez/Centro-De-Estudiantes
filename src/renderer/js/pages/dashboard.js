...
Pages.dashboard = async function() {
  ...
  // Detectar vencidos + stock bajo en paralelo
  const [, stockBajo, morososData] = await Promise.all([
    Morosos.detectarVencidos(),
    Inventario.stockBajo(),
    Morosos.listaMorosos(),
  ]);
  const nStockBajo = stockBajo.length;
  ...
  container.innerHTML = `
    ${nMorosos > 0 ? `
    <div style="background:rgba(239,68,68,0.1);border:1px solid var(--red);border-radius:var(--radius);padding:12px 16px;margin-bottom:12px;display:flex;align-items:center;gap:10px;">
      ...
      <div>
        ...
        <div style="font-size:11px;color:var(--red);opacity:0.8;">${morososData.slice(0,3).map(m=>`${m.nombre} ${m.apellido} — ${m.material_nombre||'?'}`).join(' · ')}${nMorosos>3?` · y ${nMorosos-3} más`:''}</div>
      </div>
    ...
    ${nStockBajo > 0 ? `
    <div style="background:rgba(234,179,8,0.08);border:1px solid var(--yellow);border-radius:var(--radius);padding:12px 16px;margin-bottom:12px;display:flex;align-items:center;gap:10px;">
      <span style="color:var(--yellow);font-size:16px;">📦</span>
      <div>
        <span style="font-size:13px;color:var(--yellow);font-weight:600;">${nStockBajo} producto${nStockBajo>1?'s':''} con stock bajo o sin stock</span>
        <div style="font-size:11px;color:var(--yellow);opacity:0.9;">${stockBajo.slice(0,3).map(m=>`${m.nombre} (${m.stock_actual} ${m.unidad})`).join(' · ')}${nStockBajo>3?` · y ${nStockBajo-3} más`:''}</div>
      </div>
      <button class="btn btn-sm" style="margin-left:auto;border-color:var(--yellow);color:var(--yellow);" onclick="Router.navigate('mercaderia')">Ver mercadería</button>
    </div>` : ''}
    ...
    <div class="stats-grid">
      <div class="stat-card blue">  <div class="stat-value">${pActivos}</div><div class="stat-label">Personal activo</div></div>
      <div class="stat-card green"> <div class="stat-value">${aActivos}</div><div class="stat-label">Alumnos</div></div>
      <div class="stat-card yellow"><div class="stat-value">${prestH}</div><div class="stat-label">Préstamos activos</div></div>
      <div class="stat-card purple"><div class="stat-value">${panH}</div><div class="stat-label">Panes hoy</div></div>
      <div class="stat-card blue">  <div class="stat-value">${hojasH||0}</div><div class="stat-label">Hojas entregadas hoy</div></div>
      <div class="stat-card green"> <div class="stat-value">${turnosH}</div><div class="stat-label">Turnos hoy</div></div>
    </div>
